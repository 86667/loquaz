...
impl ConfigController {
    pub fn click_add_relay_url(_ctx: &mut EventCtx, data: &mut AppState, _env: &Env) {
        // ctx.submit_command(CONNECT.with(ctx.get_external_handle()));
        data.add_relay_url();
    }

    pub fn click_copy_pk_to_clipboard(_ctx: &mut EventCtx, data: &mut AppState, _env: &Env) {
        let mut clipboard = Application::global().clipboard();
        clipboard.put_string(data.user.pk.clone());
    }
    pub fn click_generate_sk(_ctx: &mut EventCtx, data: &mut AppState, _env: &Env) {
        data.generate_sk();
    }
    pub fn click_restore_sk(_ctx: &mut EventCtx, data: &mut AppState, _env: &Env) {
        data.restore_sk();
    }
